...
2. **Read the change.** Read the committed diff with the command in the scope (add `git diff HEAD` when the scope shows uncommitted changes). Then read every changed production file in full, not only its hunks. When the diff is larger than about 3,000 lines, read it one production file at a time, highest risk first. Skim the diff of every other changed file as well. A removal the branch's purpose does not explain (a documentation section, a test, a CI step, a configuration entry) is a defect like any other, and the scope marks files with substantial removals and names the headings, definitions, tests and CI steps each one lost. For every such file, read its diff and name in the Coverage row each thing it lost. Judge each loss against the branch's stated intent in the scope: content that the commits do not account for, and that still describes something present in the repository, was removed by accident. Report it as a finding. Do not supply an intent the author did not state, and do not describe a diff you have not read.
...
Order findings by severity. With no findings, omit the Findings section and write `No actionable findings` as the outcome. List every changed production file, and every file the scope marks for a substantial removal, in the coverage table, by path; a directory of similar files may be one row written as `dir/**`. Deleted files need one row per deleted group saying what you searched for.
