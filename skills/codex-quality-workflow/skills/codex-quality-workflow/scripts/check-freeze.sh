...
cd "$repo_root"
state_file="$(git rev-parse --git-path codex-quality-workflow)/freeze.env"

if [[ ! -f "$state_file" ]]; then
  echo "check-freeze: no checkpoint; commit the implementation and invoke codex-quality-workflow with freeze" >&2
  exit 1
fi
