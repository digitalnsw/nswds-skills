...
for (let index = 0; index < args.length; index += 1) {
  if (args[index] === "--base") requestedBase = args[++index] ?? "";
  else if (!requestedBase && !args[index].startsWith("-")) requestedBase = args[index];
  else throw new Error("usage: review-scope.mjs [--base] [branch]");
}
...
const resolves = (ref) => Boolean(git(["rev-parse", "--verify", "--quiet", `${ref}^{commit}`], { optional: true }));
...
if (!base) throw new Error("could not detect a default branch locally; rerun with a branch name, for example: review-scope.mjs main");

const mergeBase = git(["merge-base", base, "HEAD"]);
const status = git(["status", "--short", "--untracked-files=all"], { optional: true });
const changed = new Set(git(["diff", "--name-only", `${mergeBase}...HEAD`], { optional: true }).split("\n").filter(Boolean));
