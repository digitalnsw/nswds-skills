---
name: nsw-government-design-system
...
If the page needs something the design system does not provide, stop and tell the user. Name the closest design system component and explain the gap. Do not build a custom version to fill it. Anything the user then approves outside the design system (analytics, a map library, a compiled stylesheet) is passed to `check` with `--allow-script` or `--allow-stylesheet`, and you name it in your report.

## 5. Check and verify

1. Run `check` on every page you created or changed, with the pinned `--version`. Fix every error until it reports `uses only NSW Design System`. Resolve or justify every warning. For framework projects, check the rendered HTML (build output or the served page), not the source.
