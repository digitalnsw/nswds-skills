...
try {
  ...
  const stateDir = resolve(repo, git(["rev-parse", "--git-path", "claude-quality-workflow"]));
  const freezeFile = join(stateDir, "freeze.env");
  if (!existsSync(freezeFile)) throw new Error("no frozen checkpoint; run /freeze-review or /quality-workflow first");
  ...
  const projectAnalysis = join(repo, ".claude", "quality-workflow", "analysis.commands");
