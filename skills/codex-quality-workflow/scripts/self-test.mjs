...
const repairReviewFinding=join(stateDir,'repair-review-finding.json');save(repairReviewFinding,fixtureReport.findings[0]);
const scopedReview=await runLane(job('scoped-repair-review',{role:'repair-review',inputs:[repairReviewFinding]}),runtime);
assert.deepEqual(JSON.parse(readFileSync(scopedReview.report)).review.assigned_finding_ids,['R-001']);
await assert.rejects(runLane(job('wrong-repair-scope',{role:'repair-review',inputs:[repairReviewFinding]}),runtime),/BLOCKED_INCOMPLETE_REVIEW/);
...
save(repairReview,{...fixtureReport,review:{...fixtureReport.review,base_sha:candidate.baseCommit,head_sha:candidate.candidateCommit,target:'repair-diff',assigned_finding_ids:['R-001']},findings:[]});
